...
export const renderTemplate = self => {
    ...
    if (self.isAnalyzing) {
        ...
        headerClass = 'pulsing searching';
    } else if (self.isLoading || (self.isStreaming && !self.currentResponse)) {
        headerText = 'Thinking';
        headerClass = 'pulsing thinking-slide-in';
    }
    // When we have response content and are not in special states, show "AI Response"

    const isCompact = self.windowHeight < 50;
    const inputPulsing = !hasResponse ? 'pulsing' : '';

    // Show dots for analyze, searching, and thinking states
    const showThinkingDots = self.isAnalyzing || self.isSearching || self.isLoading || (self.isStreaming && !self.currentResponse);
    ...
    return html`
        <div class="ask-container ${isCompact ? 'compact' : ''}">
            ...
            ${self.searchSources && self.searchSources.length > 0
                ? html`
                      <div class="search-sources-container">
                          ...
                          <div class="search-sources-list">
                              ${self.searchSources.map(
                                  source => html`
                                      <div class="search-source-item">
                                          <div class="source-logo">${self.getSourceLogo(source.domain)}</div>
            ...
            <div class="text-input-container ${!hasResponse ? 'no-response' : ''} ${!self.showTextInput ? 'hidden' : ''}">
                <div class="input-wrapper">
                    ...
                    />
                    <button
                        class="search-toggle-btn ${self.searchEnabled ? 'active' : ''}"
                        @click=${self.handleToggleSearch}
                        title="${self.searchEnabled ? 'Disable web search' : 'Enable web search'}"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="16"
